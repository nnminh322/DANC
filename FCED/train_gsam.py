...
from gsam.Step_Lr import StepLR
...
def train(local_rank, args):
    ...
    for stage in task_idx:
        ...
        event_type_ids, event_type_mask = exemplars.get_event_type_ids(
            (stage + 1) * learning_nums, device, label2idx
        )
        with torch.no_grad():
            event_type_reps = model.forward_backbone(event_type_ids, event_type_mask)[
                :, 0, :
            ]  # way x H
        ...
        if stage > 0:
            ...
            exemplars.set_exemplars(
                prev_model, exemplar_loader, len(learned_types), device, label2idx
            )
            # if not args.replay:
            ...
            for bt, batch in enumerate(tqdm(stage_loader)):
                ...
                if args.sim_event_type:
                    ...
                    for y in train_y:
                        for y_i in y:
                            if y_i != 0:
                                extended_event_type_reps.append(
                                    event_type_reps[y_i.item() - 1, :]
                                )
                    extended_event_type_reps = torch.stack(extended_event_type_reps)
                    sim_event_loss = criterion_fd(
                        extended_event_type_reps,
                        trig_feat[torch.cat(train_y) != 0],
                        torch.ones(extended_event_type_reps.shape[0]).to(device),
                    )
                ...
                # outputs[:, 0] = 0
                loss, loss_ucl, loss_aug, loss_fd, loss_pd, loss_tlcl, loss_dr = (
                    sim_event_loss,
                    0,
                    0,
                    0,
                ...
                if not args.gsam:
                    ...
                    if (args.ucl or args.tlcl) and (
                        ...
                        if args.cl_aug == "shuffle":
                            ...
                            for i in range(len(rand_num)):
                                ...
                                if args.dataset == "ACE":
                                    span_pos = (
                                        da_span[i][da_y[i].nonzero()].view(-1).unique()
                                        - 1
                                    )
                                else:
                                    span_pos = da_span[i].view(-1).unique() - 1
                                ...
                                replace_perm = torch.randperm(
                                    token_idx_ntrig.shape.numel()
                                )
                                replace_idx = token_idx_ntrig[replace_perm][
                                    : rand_num[i]
                                ]
                                new_tkn_idx = torch.randperm(len(all_tokens))[
                                    : rand_num[i]
                                ]
                                da_x[i, replace_idx] = all_tokens[new_tkn_idx].to(
                                    device
                                )
                        # if args.dataset == "ACE":
                        #     da_return_dict = model(da_x, da_masks)
                        ...
                        if args.tlcl:
                            if not (
                                ...
                                if args.aug_dropout_times > 0:
                                    dr_trig_feat, dr_y = dropout_augment(
                                        trig_feat, train_y
                                    )
                                    tlcl_feature = torch.cat(
                                        [tlcl_feature, dr_trig_feat]
                                    )
                                    tlcl_lbs = torch.cat([tlcl_lbs, dr_y])
                    ...
                    if args.pseudo_label and (stage > 0):
                        prev_model.eval()
                        with torch.no_grad():
                            prev_return_dict = prev_model(
                                train_x, train_masks, train_span
                            )
                            prev_outputs, prev_feature = (
                                prev_return_dict["outputs"],
                                prev_return_dict["context_feat"],
                            ...
                            prev_predict = prev_outputs[:, prev_learned_types].argmax(
                                -1
                            )
                            ce_y = prev_predict * (ce_y == 0).int() + ce_y
                    ...
                    if args.rep_aug != "none" and stage > 0:
                        outputs_aug, aug_y = [], []
                        for e_batch in e_loader:
                            exemplar_x, exemplars_y, exemplar_masks, exemplar_span = (
                                zip(*e_batch)
                            )
                            exemplar_radius = [
                                exemplars.radius[y[0]] for y in exemplars_y
                            ]
                            exemplar_x = torch.LongTensor(exemplar_x).to(device)
                            exemplar_masks = torch.LongTensor(exemplar_masks).to(device)
                            exemplars_y = [
                                torch.LongTensor(item).to(device)
                                for item in exemplars_y
                            ]
                            exemplar_span = [
                                torch.LongTensor(item).to(device)
                                for item in exemplar_span
                            ]
                            if args.rep_aug == "relative":
                                aug_return_dict = model(
                                    exemplar_x,
                                    exemplar_masks,
                                    exemplar_span,
                                    torch.sqrt(torch.stack(exemplar_radius)).unsqueeze(
                                        -1
                                    ),
                                )
                            else:
                    ...
                    if stage > 0 and args.distill != "none":
                        ...
                        with torch.no_grad():
                            ...
                            if (
                                args.joint_da_loss == "dist"
                                or args.joint_da_loss == "mul"
                            ):
                                outputs = torch.cat([outputs, da_outputs])
                                context_feat = torch.cat(
                                    [context_feat, da_context_feat]
                                )
                                prev_return_dict_cl = prev_model(
                                    da_x, da_masks, da_span
                                )
                                prev_outputs_cl, prev_feature_cl = (
                                    prev_return_dict_cl["outputs"],
                                    prev_return_dict_cl["context_feat"],
                        ...
                        if args.distill == "pd" or args.distill == "mul":
                            ...
                            if args.leave_zero:
                                prev_outputs[:, 0] = 0
                            prev_outputs = prev_outputs[:, prev_valid_mask_op].squeeze(
                                -1
                            )
                            cur_outputs = outputs[:, prev_valid_mask_op].squeeze(-1)
                            # prev_outputs[i].masked_fill_(prev_invalid_mask_op, torch.Tensor([float("-inf")]).squeeze(0))
